...
use {
    ...
    solana_vote_program::{
        ...
        vote_instruction,
        vote_state::{
            process_slot_vote_unchecked, process_vote_unchecked,BlockTimestamp, Lockout, TowerSync, Vote,
...
impl Tower {
    ...
    fn record_bank_vote_and_update_lockouts(
        ...
        // Process each slot in the vote
        let mut result = Ok(());
        for slot in &vote.slots {
            if let Err(e) = self.vote_state.process_next_vote_slot(*slot, pop_expired) {
                result = Err(e);
                break;
            }
        }
    ...
    pub fn is_locked_out(&self, slot: Slot, ancestors: &HashSet<Slot>) -> bool {
        ...
        if let Err(e) = vote_state.process_next_vote_slot(slot, true) {
            // If the vote would be invalid (e.g., too old), it's effectively locked out
            return true;
        }        for vote in &vote_state.votes {
    ...
    pub fn is_locked_out_including(
        ...
        // Process each slot in the including list
        for slot in including {
            // Use TowerVoteState's method instead of process_slot_vote_unchecked
            let _ = vote_state.process_next_vote_slot(*slot, true);
        }
    ...
    pub fn pop_votes_locked_out_at(&self, new_votes: &mut Vec<Slot>, slot: Slot) {
        ...
        for i in 0..new_votes.len() {
            // Replace process_slot_vote_unchecked with TowerVoteState method
            let _ = vote_state.process_next_vote_slot(new_votes[i], true);
